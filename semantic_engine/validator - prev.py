...
class MatchingValidator:
    ...
    def print_report(self, report: ValidationReport) -> None:
        ...
        # Per-pair detail
        print(f"\n{'─'*W}")
        print(f"  {'RESUME':<22} {'JD':<30} {'SCORE':>6}  {'EXPECTED':<16} {'ACTUAL':<16} {'OK'}")
        print(f"{'─'*W}")
        for rec in sorted(report.records, key=lambda r: r.resume_id):
            ...
            exp_rng = f"[{rec.expected_range[0]:.2f}-{rec.expected_range[1]:.2f}]"
            print(f"  {rname:<22} {jtitle:<30} {rec.actual_score:>6.3f}  {rec.expected_label:<16} {rec.actual_label:<16} {ok_sym}")
            ...
            "pair_results": [
                {
                    ...
                    "jd_title":       r.jd_title,
                    "job_type":       r.job_type,
                    "actual_score":   round(r.actual_score, 4),
