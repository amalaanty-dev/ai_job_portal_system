...
def _detect_section(line: str) -> str:
    """Normalizes the line by removing 'HEADER:' and checking against SECTION_HEADERS."""
    # This strips the 'HEADER:' prefix so matches like 'CORE TECHNICAL SKILLS' work
    clean_line = line.lower().replace("header:", "").strip()
    
    for section_key, header_phrase in SECTION_HEADERS:
...
def _is_certification_content(line: str) -> bool:
    """Identifies lines that belong in Certifications even if found under Education."""
    keywords = ["certificate", "certification", "certified", "coursera", "udemy", "training"]
...
def segment_resume(text: str) -> dict:
    ...
    for line in lines:
        ...
        # This must happen AFTER header/job detection but BEFORE general routing.
        if current_section == "education":
            if _is_certification_content(line):
                sections["certifications"].append(line)
            else:
                sections["education"].append(line)
            continue # Prevents the line from falling into 'other' or duplicate routing
