...
_INST_SUFFIX_RE = (
    ...
    r"IIT|NIT|IIM|XLRI|BITS|NIFT|NLU|IISER|IISc"
)
...
_DEG_STEMS = (
    ...
    r"|msc|m\.sc|master\s+of\s+science|m\.s\b|ms\b"
...
def extract_field(text):
    ...
    # P1: "degree [in/of/-] <field>"
    m = re.search(
        _DEG_STEMS
        + r"\.?\s*(?:\([^)]*\)\s*)?(?:in|of|specialization\s+in|specialising\s+in|major\s+in|-)?\s+"
        r"([a-z][a-z ,&()/]+)",
        tl
    )
    if m:
    ...
    for m in re.finditer(r"\(([^()]{3,50})\)", norm):
...
def extract_education(resume_json):
    ...
    while i < len(cleaned):
        ...
        # ── Certification block ──────────────────────────────
        if is_certification(line.lower()):
