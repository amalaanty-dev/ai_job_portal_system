...
def extract_education(resume_json):
    ...
    while i < len(cleaned):
        ...
        if is_certification(line):
            ...
            if i + 1 < len(cleaned):
                nxt = cleaned[i+1]
                # Check if next line contains metadata like '·', '/', or '20xx'
                if any(x in nxt.lower() for x in ["\u00b7", "20", "coursera", "university", "/", "|", "ibm"]):
                    cert_lookahead = nxt
            
            full_cert_text = line + " " + cert_lookahead
            certifications.append({
                "name":     line.strip(" •·(cid:127)"),
