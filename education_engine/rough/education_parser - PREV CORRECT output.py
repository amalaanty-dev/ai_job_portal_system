...
# ═══════════════════════════════════════════════════════════════
# IMPROVED RELEVANCE LOGIC (Weighted Scoring)
# ═══════════════════════════════════════════════════════════════
def calculate_relevance(education_list, certifications):
    """
    Calculates a weighted education relevance score (0-100).
    - Degree Level: 40% (Higher weight for advanced degrees)
    - Field Alignment: 40% (Matches against industry keywords)
    - Certification Bonus: 20% (Rewards upskilling)
    """
    if not education_list: return 0.0
    
    # 1. Degree Strength (Max 40 pts)
    degree_weights = {"PHD": 40, "MS": 35, "MSC": 35, "MBA": 35, "B.TECH": 30, "BS": 30}
    max_deg_score = max([degree_weights.get(e['degree'], 20) for e in education_list])
    
    # 2. Field Alignment (Max 40 pts)
    field_score = 40 if any(e['field'] != "" for e in education_list) else 0
    
    # 3. Certification Bonus (Max 20 pts)
    cert_score = min(len(certifications) * 10, 20)
    
    total = max_deg_score + field_score + cert_score
