...
class ScoreNormalizer:
    ...
    def normalize_per_jd(
        ...
        # dep lookup keyed by normalised stem so that:
        #   dep report  "resume_id": "Amala_Resume_DS_DA_2026_"
        #   ATS input   "identifiers.resume_id": "Amala_Resume_DS_DA_2026__skills"
        # both resolve to the same key after stripping the "__<suffix>" tail.
        dep_lookup = {}
        if dependency_reports:
            for d in dependency_reports:
                rid = d.get("resume_id")
                if rid:
                    dep_lookup[self._stem_resume_id(rid)] = d
    ...
    def _build_record(
        ...
        # resume_id must always be sequential R001, R002, ... format
        resume_id = f"R{candidate_index + 1:03d}"
        # job_id from identifiers (required field in output schema), canonicalized
        job_id = self._canonicalize_jd_id(idents.get("jd_id", ""))
        ...
        # dep_lookup is keyed by stemmed id — strip __skills / __<suffix> tail
        raw_resume_id = idents.get("resume_id", resume_id)
        dep_report = dep_lookup.get(self._stem_resume_id(raw_resume_id), {})
    ...
    @staticmethod
    def _stem_resume_id(rid: str) -> str:
        """
        Strip trailing ``__<word>`` suffixes from a resume identifier so that
        the dep-report key ("Amala_Resume_DS_DA_2026_") and the ATS-input key
        ("Amala_Resume_DS_DA_2026__skills") both resolve to the same stem.

        Examples:
            "Amala_Resume_DS_DA_2026__skills"  -> "Amala_Resume_DS_DA_2026_"
            "Jane_Doe_UIUX_2026__skills"       -> "Jane_Doe_UIUX_2026_"
            "Amala_Resume_DS_DA_2026_"         -> "Amala_Resume_DS_DA_2026_"
            "R001"                             -> "R001"
        """
        import re
        return re.sub(r"__\w+$", "", rid)
