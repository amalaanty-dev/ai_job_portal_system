...
def test_education_extracted():
    ...
    # New section-aware signature: (text, sections); sections is a dict from
    # _split_sections(). Empty dict triggers the regex-based fallback path.
    edu = ats_adapter._extract_education(text, {})
    edu_upper = [e.upper() for e in edu]
    assert any("B.TECH" in e or "BTECH" in e or "BE" == e or "B.E" in e for e in edu_upper) \
           or any("MSC" in e or "M.SC" in e or "MS" == e for e in edu_upper)
    # Computer Science / Data Science should also be picked up by the fallback regex
    assert any("COMPUTER SCIENCE" in e for e in edu_upper) or any("DATA SCIENCE" in e for e in edu_upper)


def test_education_extracted_from_section():
    """Section-first extraction: when 'education' section is present, use it directly."""
    sections = {
        "education": "B.Tech in Computer Science, IIT Madras, 2018\nM.Sc in Data Science, Stanford, 2020"
    }
    edu = ats_adapter._extract_education("ignored fallback", sections)
    # Section path returns the actual lines, not regex labels
    assert len(edu) >= 1
    joined = " ".join(edu).lower()
    assert "computer science" in joined or "data science" in joined
