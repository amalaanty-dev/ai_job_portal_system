...
# ---------------------------------------------------------------------
# Call queue builder (no PII lookup - Phase 3 resolves contacts)
# ---------------------------------------------------------------------
class TestCallQueueBuilder:
    def test_empty_shortlist_writes_empty_file(self, tmp_path):
        from ranking_engine.exporters.call_queue_builder import build_call_queue
        out = tmp_path / "call_queue.json"
        build_call_queue([], "default", out)
        assert out.exists()
        assert json.loads(out.read_text()) == []

    def test_no_candidate_is_dropped(self, tmp_path):
        """Even candidates with no phone are kept in the queue."""
        from ranking_engine.exporters.call_queue_builder import build_call_queue
        candidates = [
            {"candidate_id": "C1", "final_score": 85.0},
            {"candidate_id": "C2", "final_score": 78.0, "email": "c2@example.com"},
            {"candidate_id": "C3", "final_score": 72.0,
             "phone": "+91-9999900003", "name": "Carol", "email": "c3@example.com"},
        ]
        out = tmp_path / "call_queue.json"
        build_call_queue(candidates, "default", out)
        queue = json.loads(out.read_text())
        assert len(queue) == 3   # ALL three present, none silently dropped

    def test_status_reflects_phone_presence(self, tmp_path):
        from ranking_engine.exporters.call_queue_builder import build_call_queue
        candidates = [
            # Has phone -> ready
            {"candidate_id": "C1", "phone": "+91-1111111111", "final_score": 80.0},
            # No phone -> needs_contact_lookup
            {"candidate_id": "C2", "email": "c2@example.com", "final_score": 75.0},
        ]
        out = tmp_path / "call_queue.json"
        build_call_queue(candidates, "default", out)
        queue = json.loads(out.read_text())
        by_id = {e["candidate_id"]: e for e in queue}
        assert by_id["C1"]["status"] == "ready"
        assert by_id["C2"]["status"] == "needs_contact_lookup"
        assert "phone" in by_id["C2"]["missing_fields"]
        assert "phone" not in by_id["C1"]["missing_fields"]

    def test_pii_pulled_from_candidate_when_present(self, tmp_path):
        """If the candidate dict already has PII, surface it."""
        from ranking_engine.exporters.call_queue_builder import build_call_queue
        candidates = [{
            "candidate_id": "C1",
            "name":  "Alice",
        ...
        assert e["name"] == "Alice"
        assert e["phone"] == "+91-9999900001"
        assert e["email"] == "alice@example.com"
        assert e["missing_fields"] == []

    def test_day13_shape_produces_needs_lookup(self, tmp_path):
        """Real Day 13 ATS JSON has no contact info - all entries should
        end up as needs_contact_lookup but NOT be dropped."""
        from ranking_engine.exporters.call_queue_builder import build_call_queue
        candidates = [
            {"candidate_id": "Resume_A_skills", "final_score": 82.0,
             "job_role": "ai specialist"},
            {"candidate_id": "Resume_B_skills", "final_score": 75.5,
             "job_role": "ai specialist"},
        ]
        out = tmp_path / "call_queue.json"
        build_call_queue(candidates, "ai_specialist", out)
        queue = json.loads(out.read_text())
        assert len(queue) == 2
        assert all(e["status"] == "needs_contact_lookup" for e in queue)
        assert all(e["phone"] is None for e in queue)
        assert all(e["job_role"] == "ai specialist" for e in queue)  # candidate-level role wins
        assert all(e["retry_count"] == 0 for e in queue)

    def test_alternative_phone_keys(self, tmp_path):
        """Accept phone_number, contact_number, mobile as phone aliases."""
        from ranking_engine.exporters.call_queue_builder import build_call_queue
        candidates = [
            {"candidate_id": "C1", "phone_number":   "+91-1", "final_score": 80},
            {"candidate_id": "C2", "contact_number": "+91-2", "final_score": 78},
            {"candidate_id": "C3", "mobile":         "+91-3", "final_score": 75},
        ]
        out = tmp_path / "call_queue.json"
        build_call_queue(candidates, "default", out)
        queue = json.loads(out.read_text())
        assert all(e["status"] == "ready" for e in queue)
        assert {e["phone"] for e in queue} == {"+91-1", "+91-2", "+91-3"}