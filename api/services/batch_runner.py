...
def shortlist_for_job(job_id: str, threshold: float = 70.0, top_n: int | None = None) -> dict[str, Any]:
    """Pull stored scores for a job, rank by score, return ONLY shortlisted candidates."""
    rows = storage.get_scores_for_job(job_id)
    rows.sort(key=lambda r: r["final_score"], reverse=True)

    # Only include candidates who passed the threshold
    shortlisted = []
    rank = 0
    for r in rows:
        if r["final_score"] >= threshold:
            ...
            shortlisted.append({
                ...
                "score": r["final_score"],
                "status": "Shortlisted",  # always Shortlisted — rejected are excluded
                "rank": rank,
            })
    ...
    return {
        ...
        "candidates": shortlisted,  # only shortlisted, rejected excluded entirely
    }