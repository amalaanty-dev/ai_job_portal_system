...
# Digits excluded from stop-chars so titles like "Data Analyst 2",
# "Level 3 Engineer", "ISO 27001 Analyst" are captured correctly
_FIELD = r'[^,|()\n–—]+'
...
_PATTERNS = [
    ...
    # Format 4 → Role Company Month YYYY – Month YYYY  (space-separated blob)
    # Handles: "junior data analyst intern northwestern medicine jun 2023 dec 2023"
    # Captures role-like text, then company-like text, then two date tokens
...
def _resolve_end_date(end_date_str):
    """
    Robust end date resolver for resume experience parsing
    """

    if not end_date_str:
        return None, False

    end_date_str = str(end_date_str).strip().lower()

    # CASE 1: Present / Current job
    if end_date_str in ["present", "current", "now"]:
        return datetime.today(), True

    # CASE 2: Real date parsing
    try:
        parsed_date = parser.parse(end_date_str, fuzzy=True)
