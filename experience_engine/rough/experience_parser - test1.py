...
def _resolve_end_date(raw: str) -> date | None:
    """Resolve end date string to a date object, defaulting to today for 'Present'."""
    if not raw:
        return None
...
# ── TOTAL EXPERIENCE (MERGED) ─────────────────────────────────────────
def calculate_total_experience(experiences: list[dict]) -> int:
    """Return total unique months, merging overlapping ranges to avoid double-counting."""
    if not experiences: return 0

    intervals = []
    for exp in experiences:
        try:
            intervals.append((date.fromisoformat(exp["start_date"]), date.fromisoformat(exp["end_date"])))
        except: continue
...
def detect_gaps(experiences: list[dict], min_gap_months: int = 3) -> list[dict]:
    ...
    sorted_exp = sorted(experiences, key=lambda x: date.fromisoformat(x["start_date"]))
    gaps = []
...
# ── 🔥 OVERLAP DETECTION (REPAIRED) ───────────────────────────────────
def detect_overlaps(experiences: list[dict]) -> list[dict]:
    """Detect concurrent roles using a rigorous pairwise comparison."""
    if len(experiences) < 2:
        return []

    # Sort to ensure we always compare an earlier job (A) to a later job (B)
    sorted_exp = sorted(experiences, key=lambda x: date.fromisoformat(x["start_date"]))
    overlaps = []
    ...
    for i in range(len(sorted_exp)):
        for j in range(i + 1, len(sorted_exp)):
            ...
            # REPAIR: An overlap exists if Job B starts before Job A ends
            if b_start < a_end:
                # Calculate the exact range of the overlap
...
def build_structured_experience(text: str) -> dict:
    """Full Day-10 deliverable — orchestrates extraction, totals, and timeline analysis."""
