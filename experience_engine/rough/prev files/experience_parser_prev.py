...
_PATTERNS = [
    ...
    # Format 4 → Fallback for unstructured text blocks: Role Company DateRange
...
def calculate_total_experience(experiences: list[dict]) -> int:
    ...
    for next_start, next_end in intervals[1:]:
        if next_start <= curr_end:
            # Overlap: extend the current end date if the next role ends later
    ...
    # Sum months using the updated inclusive calculation from date_utils
...
def detect_gaps(experiences: list[dict], min_gap_months: int = 3) -> list[dict]:
    ...
    for i in range(1, len(sorted_exp)):
        ...
        # Adjusted for inclusive logic: gap is distance between end of Role A and start of Role B
