...
# ----------------------------
# FIELD FIX (STRONG - MULTILINE)
# ----------------------------
def infer_field_from_education_block(education, resume_text):
    ...
    for e in education:
        ...
        if any(k in text for k in ["finance", "accounting", "banking"]):
            e["field"] = "FINANCE & MARKETING"
        elif any(k in text for k in ["marketing", "sales", "branding"]):
            e["field"] = "MARKETING"
        elif any(k in text for k in ["business administration"]):
            e["field"] = "BUSINESS ADMINISTRATION"
        elif any(k in text for k in ["data", "machine learning", "ai"]):
            e["field"] = "DATA SCIENCE"
        else:
            e["field"] = "GENERAL"
...
def fix_institution_name(inst, full_text):
    ...
    match = re.search(
        r"([A-Z][a-zA-Z &,.]+(?:College|University|Institute)[a-zA-Z &,.]*)",
        full_text
    )
...
def run_education_pipeline(resume_json, jd_json):
    ...
    # ----------------------------
    # FIX FIELD (CRITICAL FIX)
    # ----------------------------
    education = infer_field_from_education_block(education, resume_text)
