...
def run_batch() -> None:
    ...
    for resume in resumes:
        ...
        output = {
            ...
            # FIX: include all section scores in output for debugging
            "section_scores": best.to_dict()["section_scores"],
            "gaps":           best.gaps,
            # FIX: record which job_type was inferred / used
            "job_type_used":  infer_job_type(best.jd_title, jds[0] if jds else {}),
        }
        ...
        print(f"  📄 {resume.get('name','?'):<20} → best: "
              f"{best.jd_title:<30}  {best.overall_score:.3f}  "
