...
def _resolve_jd_roles(jd_data: dict) -> tuple:
    ...
    for key in _JD_ROLE_KEYS:
        val = jd_data.get(key)
        if val:
            roles = [val] if isinstance(val, str) else val
            return roles, key
    return [], None
...
def main():
    ...
    for resume_file in resume_files:
        ...
        if isinstance(resume_exp, list):
            ...
            if resume_exp and isinstance(resume_exp[0], dict):
                ...
                dropped = len(resume_exp) - len(experiences)
                if dropped:
                    logger.warning(
        ...
        if not experiences:
            logger.warning("⚠️ '%s' — no valid experiences extracted", resume_name)

        # ── JD LOOP ───────────────────────────────────────────────────
        for jd_name, jd_data in jd_cache.items():

            # Skip pre-validated invalid JDs
            ...
            # Build summary
            try:
                summary = build_experience_summary(experiences, jd_roles)
                final_output = format_day10_output(experiences, summary)
            except Exception as e:
                logger.warning("❌ build_experience_summary failed (%s × %s): %s", resume_name, jd_name, e)
                skipped += 1
                continue
            ...
            # months   = summary.get("total_experience_months", 0)
            ...
            if is_empty:
                logger.warning(
                    "⚠️ %s × %s → empty summary — experiences=%d | jd_roles=%d | check keys/date format",
                    resume_name, jd_name, len(experiences), len(jd_roles)
                )
                empty_result += 1
                continue                             # skip disk write entirely

            # Write output only when summary has real data
            ...
            try:
                ...
                logger.warning("❌ Failed to write output for %s × %s: %s", resume_name, jd_name, e)
                skipped += 1
                continue
    ...
    logger.info("Empty Result : %d  ← fix resume/JD keys or date format if > 0", empty_result)
