...
def _pct(score: float) -> float:
    """Convert raw cosine score [0,1] → percentage float XX.XX"""
...
def _build_eval_report(
    ...
    """
    Build a full matching evaluation report for one resume against all JDs.
    Saved to semantic_evaluation_reports/ by run_batch().

    Includes:
      • summary statistics (best match, avg score, label distribution)
      • full ranked list of all JD matches with section scores + gaps
    """
    ...
    for r in all_results:
        jd       = jd_map.get(r.jd_id, {})
        job_type = jd.get("job_type", infer_job_type(r.jd_title, jd))
        ss       = r.to_dict()["section_scores"]   # already in % format from to_dict()
        ...
        matches.append({
            ...
            "jd_title":       r.jd_title,
            "job_type":       job_type,
            "semantic_score": _pct(r.overall_score),
    ...
    best   = all_results[0]    # list is already sorted descending

    label_counts = {
        "Strong Match":  sum(1 for r in all_results if r.match_label == "Strong Match"),
        "Good Match":    sum(1 for r in all_results if r.match_label == "Good Match"),
        "Partial Match": sum(1 for r in all_results if r.match_label == "Partial Match"),
        "Weak Match":    sum(1 for r in all_results if r.match_label == "Weak Match"),
        "Mismatch":      sum(1 for r in all_results if r.match_label == "Mismatch"),
    }

    summary = {
        "total_jds_evaluated": len(all_results),
        "best_match_jd":       best.jd_title,
        "best_match_score":    _pct(best.overall_score),
        "best_match_label":    best.match_label,
        "avg_semantic_score":  round(sum(scores) / len(scores) * 100, 2) if scores else 0.0,
...
def _load_json_dir(directory: str, kind: str = "auto") -> list:
    """
    Load all JSON files from a directory.

    Args:
        directory : path to folder
        kind      : "resume", "jd", or "auto" (detects from keys)

    Injects:
        "name"     for resumes  — from other[0] via extract_resume_name()
        "title"    for JDs      — from role[0]  via extract_jd_title()
        "job_type" for JDs      — inferred via infer_job_type() if missing
    """
    items = []
        ...
        if fname.endswith(".json"):
            ...
            try:
                with open(path, encoding="utf-8") as f:
                    ...
                    # ── Auto-detect kind from JSON keys ──
                    is_resume = kind == "resume" or (
                        kind == "auto" and ("skills" in data or "experience" in data)
                    )
                    is_jd = kind == "jd" or (
                        kind == "auto" and ("role" in data or "skills_required" in data)
                    )
...
def run_batch() -> None:
    ...
    resumes = _load_json_dir(RESUME_DIR, kind="resume")
    jds     = _load_json_dir(JD_DIR,     kind="jd")
    ...
    jd_map = {jd.get("id", ""): jd for jd in jds}
    ts     = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    for resume in resumes:
        ...
        # ── 1. Semantic output → data/semantic_outputs/ ─────────
        output = {
            "resume_id":   resume_id,
            "resume_name": resume_name,
            "best_match": {
                ...
                "semantic_score": _pct(best.overall_score),
            ...
            "gaps":           best.gaps,
            "job_type_used":  jd_map.get(best.jd_id, {}).get(
                                  "job_type",
                                  infer_job_type(best.jd_title, jd_map.get(best.jd_id, {}))
                              ),
        }

        out_path = os.path.join(OUTPUT_DIR, f"{resume_id}_semantic.json")
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(output, f, indent=4)

        # ── 2. Evaluation report → data/semantic_evaluation_reports/ ──
        eval_report = _build_eval_report(resume, all_results, jd_map, ts)
        eval_path   = os.path.join(EVAL_DIR, f"{resume_id}_evaluation.json")
        with open(eval_path, "w", encoding="utf-8") as f:
            json.dump(eval_report, f, indent=4)

        print(f"  📄 {resume_name:<22}  score: {_pct(best.overall_score):>6.2f}%"
              f"  label: {best.match_label:<14}  best JD: {best.jd_title}")

    print(f"\n  ✅ Semantic outputs   → {OUTPUT_DIR}")
    print(f"  ✅ Evaluation reports → {EVAL_DIR}\n")
