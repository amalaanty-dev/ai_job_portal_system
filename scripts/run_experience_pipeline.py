...
# ── Paths ─────────────────────────────────────────────────────────────
RESUME_FOLDER = os.getenv("RESUME_FOLDER", "data/resumes/parsed_resumes/JSON/")
...
# ── Experience key variants ───────────────────────────────────────────
# Priority order matters:
#   1. "roles"             → parsed resume format (clean structured dicts)
#   2. "experience_section"→ sectioned resume (role_header + duties)
#   3. "experience"        → sectioned resume (role_header + duties)
#                            NOTE: some parsed resumes use "experience" as a
#                            float (total years) — handled in type checks below
#   4. everything else     → older/alternate key names
_EXP_KEYS = [
    "roles",                    # parsed resume  → list of clean dicts
    "experience_section",       # sectioned resume → list of {role_header, duties}
    "experience",               # sectioned resume → list of {role_header, duties}
                                # (parsed resume uses this as a float — skipped below)
    "work_experience",
...
# ── Required keys for fully-structured experience dicts ───────────────
_REQUIRED_EXP_KEYS = {"job_title", "company", "start_date", "end_date"}


# ── Helper: resolve experience field ─────────────────────────────────
def _resolve_exp_field(resume_data: dict) -> tuple:
    """
    Try all known experience key variants in priority order.

    Skips keys whose value is not a list (e.g. the parsed resume stores
    total years as  "experience": 5.2  — a float, not usable here).
...
def _resolve_jd_skills(jd_data: dict) -> list:
    ...
    When empty, the skill component is omitted from scoring rather
    than zeroing the whole score.
    """
...
# ── Helper: detect which experience list shape we have ───────────────
def _is_role_header_format(exp_list: list) -> bool:
    """
    Returns True if the list items use the sectioned-resume
    role_header + duties format.
    """
    if not exp_list or not isinstance(exp_list[0], dict):
        return False
    return "role_header" in exp_list[0]


def _is_structured_format(exp_list: list) -> bool:
    """
    Returns True if the list items are already fully structured dicts
    (have job_title / company / start_date / end_date).
    """
    if not exp_list or not isinstance(exp_list[0], dict):
        return False
    return _REQUIRED_EXP_KEYS.issubset(exp_list[0])
...
def main():
    ...
    for resume_file in resume_files:
        ...
        logger.debug(
            "🔑 '%s' — experience key resolved: '%s' (%d items)",
            resume_name, matched_key, len(resume_exp),
        )

        # ── Handle all input shapes ───────────────────────────────────
        #
        # Shape A — parsed resume "roles" key
        #           Items already have job_title/company/start_date/end_date
        #           → validate required keys → clean → use directly
        #
        # Shape B — sectioned resume "experience" / "experience_section" key
        #           Items have role_header + duties
        #           → parse role_header → extract clean dicts
        #
        # Shape C — list of raw strings
        #           → join → regex parse
        #
        # Shape D — single raw text string
        #           → regex parse
        ...
        if isinstance(resume_exp, list) and resume_exp:
            ...
            if _is_structured_format(resume_exp):
                # Shape A — fully structured dicts (parsed resume)
                raw_experiences = [
            ...
            elif _is_role_header_format(resume_exp):
                # Shape B — sectioned resume role_header format
                logger.debug("'%s' — detected role_header format", resume_name)
                experiences = extract_from_role_headers(resume_exp)

            elif isinstance(resume_exp[0], str):
                # Shape C — list of raw strings
                ...
                logger.warning(
                    "⚠️ '%s' — unrecognised list item type: %s",
                    resume_name, type(resume_exp[0]),
                )
        ...
        # ── Early exit — nothing usable ────────────────────────────────
        if not experiences:
            logger.warning(
                "⚠️ '%s' — no valid experiences extracted (key='%s'); skipping all JDs",
                resume_name, matched_key,
            )
            skipped += 1
            continue

        logger.debug(
            "✔ '%s' — %d experience(s) ready for scoring",
            resume_name, len(experiences),
        )
        ...
        for jd_name, jd_data in jd_cache.items():
            ...
            if not jd_skills:
                logger.debug(
                    "ℹ️  JD '%s' — no skills key found; skill component skipped in scoring",
                    jd_name,
                )

            # Build summary + format output
            try:
            ...
            # ── Skip empty summaries — don't write to disk ─────────────
