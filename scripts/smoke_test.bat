...
REM Extract job_id using py (not python)
for /f "tokens=*" %%A in ('py -c "import json; d=json.load(open(\"jd_resp.json\")); print(d.get(\"job_id\",\"\"))"') do (
    set JOB_ID=%%A
)
...
for /f "tokens=*" %%A in ('py -c "import json; d=json.load(open(\"resume_resp.json\")); print(d.get(\"resume_id\",\"\"))"') do (
    set RESUME_ID=%%A
)
for /f "tokens=*" %%A in ('py -c "import json; d=json.load(open(\"resume_resp.json\")); print(d.get(\"candidate_id\",\"\"))"') do (
    set CAND_ID=%%A
)
